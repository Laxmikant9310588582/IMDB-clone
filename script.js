...
async function singleMovie() {
    ...
    <span class="italics-text"><i>${data.Year} &#x2022; ${data.Country} &#2022; Rating - <span
        style="font-size: 18px; font-weight: 600;">${data.imdbRating}</span>/10 </i></span>
    <ul class="details-ul">
    <li><strong>Actors: </strong>${data.Actors}</li>
    <li><strong>Director: </strong>${data.Director}</li>
    <li><strong>Writers: </strong>${data.Writers}</li>
    </ul>
    <ul class="details-ul">
...
async function addTofavorites(id){
    ...
    localStorage.setItem(Math.random().toString(36).slice(2,7),id);//math.random for the unique key and value p 
...
async function displayMovieList(movies){
    var output = '';
    //Traversing over the movies list which is passed as an argument to our finction 
    for ( i of movies)  {
...
async function favoritesMovieLoader (){
    ...
    for (i in localStorage){
        ...
        if (id !=null){
            ...
            <i class="fa-solid fa-trash" style="cursor:pointer;"onClick=removeFromfavorites('${id}')>
            </div>
            </div>
            </div>
            </div>
